            ...
            <div className="space-y-5">
                ...
                <div className="rounded-xl border border-border/60 bg-white p-5 shadow-sm">
                    ...
                    <div className="flex items-center gap-4 border-t border-border/50 pt-3">
                        <button
                            onClick={toggleLike}
                            className={`flex items-center gap-1.5 text-sm transition-all ${post.is_liked ? "text-red-500 font-medium" : "text-muted-foreground hover:text-red-500"}`}
                        >
                ...
                {/* Comment Form */}
                <form onSubmit={submitComment} className="rounded-xl border border-border/60 bg-white p-5 shadow-sm">
                    {replyTo && (
                        <div className="mb-3 flex items-center gap-2 text-sm text-primary">
                            <Reply className="h-4 w-4" />
                            Membalas {replyTo.name}
                            <button type="button" onClick={cancelReply} className="ml-auto text-xs text-muted-foreground hover:text-foreground">
                                Batal
                            </button>
                        </div>
                    )}
                ...
                <div className="space-y-3">
                    {post.comments.map((comment) => (
                        <div key={comment.id} className="rounded-xl border border-border/60 bg-white p-4 shadow-sm">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-3 mt-2">
                                        <button onClick={() => startReply(comment)} className="text-xs text-muted-foreground hover:text-primary flex items-center gap-1">
                                            <Reply className="h-3.5 w-3.5" /> Balas
                                        </button>
                                        {comment.user.id === user.id && (
                                            <button onClick={() => deleteComment(comment.id)} className="text-xs text-muted-foreground hover:text-destructive">
                                                Hapus
                                            </button>
                                        )}
                                    </div>
                                        ...
                                        <div key={reply.id} className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                {reply.user.id === user.id && (
                                                    <button onClick={() => deleteComment(reply.id)} className="text-xs text-muted-foreground hover:text-destructive mt-1">
                                                        Hapus
                                                    </button>
                                                )}
                                            </div>
